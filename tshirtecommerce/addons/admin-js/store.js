design.store = {
	...
	ideas:{
		...
		add: function(data, imgs, areaDesign){
			...
			var html = '<div class="box-art box-idea">'
					 ...
					 +			imgs
					 +			'<img src="'+data.thumb+'" class="thumb-idea" id="thumb-idea-'+data.id+'" alt="'+data.title+'" style="width:'+areaDesign.width+'px; height:auto;max-height:'+areaDesign.height+'px; left:'+areaDesign.left+'px; z-index:'+areaDesign.zindex+';">'
					 +		'</a>'
	...
	admin:{
		...
		ini: function(){
			...
			jQuery('#product-price').append('<a href="#create-design-idea" class="btn btn-primary" onclick="design.store.admin.view()">Create Design</a>');
		},
		categories: function(){
			jQuery.ajax({
		...
		post: function(){
			...
			datas.productColor 	= productColor;
	...
	jQuery(document).ready(function(){
		...
		if(jQuery('.idea-fields').length > 0)
		{
			jQuery(document).on('select.item.design', function(event, e){
				var position = jQuery('.labView.active').offset();
				var width = jQuery('.labView.active').width();
				var top = position.top;
				var left = width + position.left;
				jQuery('.idea-fields').css({'top':top+'px', 'left':left+'px', 'display':'block'});

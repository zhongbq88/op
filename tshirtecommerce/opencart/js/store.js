...
var payment = {
	key: function(api, ids, design_id){
		...
		var url = store_ajaxurl + 'index.php?route=tshirtecommerce/store/store_ajax_key&api_key='+api+'&arts='+ids+'&order_id='+design_id;
		jQuery.ajax({
			url: url,
	...
	load: function(e){
		...
		jQuery('.arts-store-payment').html('<a href="javascript:void(0);" onclick="payment.cancel()">x</a><iframe id="store-art-payment" scrolling="no" frameborder="0" noresize="noresize" width="100%" height="600px" src="http://store.9file.net/api/index/'+id+'"></iframe>');
		if(jQuery('#arts-store-mask').length == 0)
		{

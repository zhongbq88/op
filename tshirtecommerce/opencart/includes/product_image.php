...
switch ($tshirtecommerce_allow_images) {
	case 2:
		$data['images'] = $tsImages;
		if (count($tsImages) > 0) {
			$data['popup'] = $tsImages[0]['popup'];
			$data['thumb'] = $tsImages[0]['thumb'];
		}
		break;

	case 3:
		$data['images'] = array_merge($data['images'], $tsImages);
		break;
...
$data['tshirtecommerce_color'] = $default_tcolor;
$data['tcolor_link'] = $this->url->link(
	'product/product', 
	'product_id='.(int)$this->request->get['product_id']
);
$data['tshirtecommerce_designer_link'] = $this->url->link(
	'tshirtecommerce/designer', 
	'parent_id='.(int)$this->request->get['product_id']
);
if (isset($data['design_product_id']) && !empty($data['design_product_id'])) {
	$data['tshirtecommerce_designer_link'] .= '&product_id='.$data['design_product_id'];
}
if (isset($tshirtecommerce_color) && !empty($tshirtecommerce_color)) {
	$data['tshirtecommerce_designer_link'] .= '&color='.$tshirtecommerce_color;
}

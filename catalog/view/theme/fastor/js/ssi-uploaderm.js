		...
		function ajaxLoopRequestAddCart(thisS,selected,selectIndex) {
		 ...
		 $uploadBox.on('click', '.ssi-editBtn', function (e) { //remove the file from list
			...
			jQuery.ajax({
				...
				if(data.error==0){
					var product = thisS.options.products[index/thisS.options.uploadSize];
...
//----------------------------UPLOADFILES------------------------------------
						...
						if (data.status == 1)
							{
								...
								var uploadImage = element.get(0).getAttribute("src");
								var design = thisS.options.products[0];
								var len = thisS.options.products.length;
								getResultImage(thisS,uploadImage,ii,design,0,len,function(dataURL){
									element.attr('src',dataURL);
								}); 
								
								for(var i=1;i<len;i++){
									var design = thisS.options.products[i];
									getResultImage(thisS,uploadImage,ii,design,i,len,function(dataURL){
										element.find('#ssi-imgToUploadTable'+ ii).after(createView(i,dataURL));
										//element.attr('src',dataURL);
									}); 
								}
							}
	...
	var createView =  function(index,dataURL){
		return '<table class="ssi-imgToUploadTable ssi-completed"><tbody><tr><td class="ssi-upImgTd"><img class="ssi-imgToUpload" id="ssi-imgToUpload"'+index+' src="'+dataURL+'"></td></tr><tr><td></td></tr><tr><td><div style="display:block" id="ssi-success'+index+'"><input type="checkbox" checked="" id="checkbox[]" name="selected[]" value="'+index+'" style="margin: 5px 0 5px 5px;padding: 0;width:20px;height:20px;"><button id="ssi-editBtn'+index+'" data-edit="'+index+'" class=" ssi-button success  ssi-editBtn" style="display:block;margin: 5px 0 5px 0px;padding: 0;float:left; width:20px;height:20px;"><span class="fa fa-pencil"></span></button></div></td></tr></tbody></table>';
	}
	
	 var getResultImage = function (thisS, uploadImage,index,design,pindex,len,callback) {//return a message label
		...
		ctx.fillStyle='#fff';
		ctx.fill();
		function drawing(thisS, design, data,n,ctx,c,uploadImage,index,pindex,len,callback){
			    //console.log(data);
				var img=new Image;
				img.src=data;	
				img.onload=function(){
		...
		drawing(thisS, design, uploadImage,0,ctx,c,element,index,pindex,len,callback);

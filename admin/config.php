<?php
// HTTP
define('HTTP_SERVER', 'http://52.36.35.1/admin/');
define('HTTP_CATALOG', 'http://52.36.35.1/');

// HTTPS
define('HTTPS_SERVER', 'http://52.36.35.1/admin/');
define('HTTPS_CATALOG', 'http://52.36.35.1/');

// DIR
define('DIR_APPLICATION', '/opt/bitnami/apps/opencart/admin/');
define('DIR_SYSTEM', '/opt/bitnami/apps/opencart/system/');
define('DIR_IMAGE', '/opt/bitnami/apps/opencart/image/');
define('DIR_LANGUAGE', '/opt/bitnami/apps/opencart/admin/language/');
define('DIR_TEMPLATE', '/opt/bitnami/apps/opencart/admin/view/template/');
define('DIR_CONFIG', '/opt/bitnami/apps/opencart/system/config/');
define('DIR_CACHE', '/opt/bitnami/apps/opencart/system/storage/cache/');
define('DIR_DOWNLOAD', '/opt/bitnami/apps/opencart/system/storage/download/');
define('DIR_LOGS', '/opt/bitnami/apps/opencart/system/storage/logs/');
define('DIR_MODIFICATION', '/opt/bitnami/apps/opencart/system/storage/modification/');
define('DIR_UPLOAD', '/opt/bitnami/apps/opencart/system/storage/upload/');
define('DIR_CATALOG', '/opt/bitnami/apps/opencart/catalog/');

// DB
define('DB_DRIVER', 'mysqli');
define('DB_HOSTNAME', 'localhost');
define('DB_USERNAME', 'root');
define('DB_PASSWORD', 'shopify@8899');
define('DB_DATABASE', 'customdr');
define('DB_PORT', '3306');
define('DB_PREFIX', 'oc_');
